...
export default createStore({
  ...
  mutations: {
    ...
    upsertPlayer (
      state,
      { id, username, isPlaying, invitations, gameId, status }
    ) {
      const player = state.onlinePlayers.find((player) => player.id === id)
      if (!player) {
        state.onlinePlayers.push({
          id,
          username,
          isPlaying,
          invitations,
    ...
    upsertRoom (state, { id, gameId, black, white, userB, userW, status }) {
      const userBlack = state.onlinePlayers.find((player) => player.id === black)
      const userWhite = state.onlinePlayers.find((player) => player.id === white)
  ...
  actions: {
    ...
    async finishUserGame ({ commit, dispatch }, { id1, id2 }) {
      await updateDoc(
        doc(db, 'players', 'P7fvewsEb8HVtkm6lDk0', 'online', String(id1)),
        {
          gameId: null,
          isPlaying: false,
          invitations: null
      ...
      await updateDoc(
        doc(db, 'players', 'P7fvewsEb8HVtkm6lDk0', 'online', String(id2)),
        {
          gameId: null,
          isPlaying: false,
          invitations: null
    ...
    async removingUsersPlaying ({ commit, dispatch }, { id }) {
      await updateDoc(
        doc(db, 'rooms', 'vkuR1EmCBLWTOZw07arF', 'online', String(id)),
        {
          status: false
        },
        { merge: true }
    ...
    async logoutUser ({ commit, dispatch }, { id }) {
      await updateDoc(
        doc(db, 'players', 'P7fvewsEb8HVtkm6lDk0', 'online', String(id)),
        {
          status: false
        },
        { merge: true }
    ...
    async creatingRoom ({ commit, dispatch }, room) {
      ...
      const gameRef = doc(
        collection(db, 'rooms', 'vkuR1EmCBLWTOZw07arF', 'online')
      )
      batch.set(gameRef, {
        date: Timestamp.now()
      })
    ...
    async connectOnlineUsers ({ commit, state, dispatch }) {
      const q = query(
        collection(db, 'players', 'P7fvewsEb8HVtkm6lDk0', 'online')
      )
    ...
    async movePiece ({ state }, { id, xpos, ypos }) {
      await updateDoc(
        doc(db, 'games', state.gameId, 'pieces', String(id)),
        {
          xpos,
          ypos
        },
    ...
    async getPlayersGame ({ state, commit }) {
      try {
        const querySnapshot = await getDocs(
          collection(db, 'games', 'hCpVk86Kuv55blgCeVcB', 'pieces')
        )
